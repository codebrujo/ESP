...
struct historicalData {
  ...
  float sensor_humidity[HISTORY_VOLUME];                  // Humidity data
...
void printCurrentToSerial() {
  ...
  Serial.print(", H ");
  Serial.println(current_status.sensor_humidity);
}
...
void printToSerial() {
  ...
  for (i = 0; i < HISTORY_VOLUME; i++) // цикл по строкам
  {
    ...
    dtostrf(historical_data.sensor_humidity[i], 3, 3, Str);       //float to string
    Serial.write(Str, sizeof(Str) / sizeof(char));
    Serial.println("");
  }
}

char * createBlynkString(float sensor_temp, float sensor_humidity) {
  int i, ccode;
  int shiftT = 3;
  int shiftP = 12;
  char Str[10];
  ...
  dtostrf(sensor_humidity, 5, 1, Str);
  for (int i = 0; i < sizeof(Str) / sizeof(char); i++) {
    ccode = (int)Str[i];
    if (ccode == 0 || ccode == 32) {
      continue;
...
void sendCurrentToBlynk() {
  ...
  Blynk.virtualWrite(V2, createBlynkString(current_status.sensor_temp, current_status.sensor_humidity));
...
void updateHistory() {
  ...
  historical_data.sensor_humidity[historical_data.current_index] = current_status.sensor_humidity;
